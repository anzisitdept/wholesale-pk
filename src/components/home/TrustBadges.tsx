        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {badges.map((badge) => (
            <div key={badge.id} className="flex flex-col items-center text-center group">
              <div className="w-32 h-32 rounded-full border-[3px] border-[#6f0c07] p-4 flex items-center justify-center mb-6 group-hover:scale-105 transition-transform duration-300 shadow-sm bg-[#f5f5f5]">
                <img src={badge.img} alt="Trust Icon" className="w-full h-full object-contain mix-blend-multiply" />
              </div>
              <h3 className="font-bold text-[#6f0c07] text-sm whitespace-pre-line leading-snug">
                {badge.title}
              </h3>
            </div>
          ))}
